...
public class searchandsort {
	...
	public static int[] radix(int[] manta) {
		...
		int a = 0;
		for(int i = 0; i < 3; i++) {
			copy = new int[manta.length];
			a = 0;
			for(int j = 0; j < 10; j++) {
				for (int k = 0; k < manta.length; k++) {					
					if ((manta[k]/((int)Math.pow(10, i)))%10 == j) {
						copy[a] = manta[k];
						a++;
					}
				}
	...
	public static int[] quickSort(int[] manta, int startPoint, int endPoint) {
	    int low = startPoint;
	    int high = endPoint;
	    while (low < high) {
	        while (low < high && manta[high] >= manta[low]) 
	        	high--;
	        if (low < high) {
	            int temp = manta[high];
	        ...
	        while (low < high && manta[low] <= manta[high]) 
	        	low++;
	        if (low < high) {
	            int temp = manta[high];
	            manta[high] = manta[low];
	    ...
	    if (low - startPoint > 1) 
	    	quickSort(manta, startPoint, low - 1);
	    if (endPoint - high > 1) 
	    	quickSort(manta, high + 1, endPoint);
	    return manta;
	}
	
	public static void mergeSort(int[] manta) {
		int[] sting = new int[manta.length];
		
		mergeSort(manta, sting, 0, manta.length-1);
	}
	
	public static void mergeSort(int[] manta, int[] sting, int s, int e) {
		if(s >= e)
			return;
		
		int m = (s+e)/2;
		
		mergeSort(manta, sting, s, m);
		mergeSort(manta, sting, m+1, e);
